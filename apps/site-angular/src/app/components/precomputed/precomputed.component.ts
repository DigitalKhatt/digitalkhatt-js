...
@Component({
  selector: 'app-precomputed-component',
  templateUrl: './precomputed.component.ts.html',
  styleUrls: ['./precomputed.component.ts.scss'],
  host: {
...
export class PrecomputedComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  static DEFAULT_PAGE_SIZE = { width: 255, height: 410, marginWidth: PrecomputedComponent.DEFAULT_SCALE * 400 };
  static DFAULT_FONT_SIZE = PrecomputedComponent.DEFAULT_PAGE_SIZE.width / (17000 / 1000);

  pageSize = PrecomputedComponent.DEFAULT_PAGE_SIZE
  ...
  constructor(@Inject(MUSHAFLAYOUTTYPE) mushafLayoutType: MushafLayoutType,
    ...
    switch (mushafLayoutType) {
      case MushafLayoutType.OldMadinah:
        ...
        this.defaultFontSize = PrecomputedComponent.DEFAULT_PAGE_SIZE.width / (16400  / 1000);
        break;
      ...
      default:
        ...
        this.defaultFontSize = PrecomputedComponent.DEFAULT_PAGE_SIZE.width / (16200 / 1000);
        break;
    }
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      ...
      this.ngZone.runOutsideAngular(async () => {
        ...
        const response = await fetch("assets/layouts/madina.json")
        
        this.layout = new LayoutService(await response.json());
        ...
        document.fonts.load("12px oldmadina").then(() => {
          ...
          this.pageElements.forEach((page, index) => {
            ...
            this.views[index] = new PageView(page.nativeElement, index,
              this.lineJustify.nativeElement,
