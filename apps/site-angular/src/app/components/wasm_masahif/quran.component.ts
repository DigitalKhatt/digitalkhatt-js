import { Component, AfterViewInit, OnInit, HostListener, OnDestroy, ViewChildren, QueryList, ElementRef, ViewChild, NgZone, TemplateRef, Inject } from '@angular/core';
...
export interface PageFormat {
  ...
  height: number,
  fontSize: number,
  // Set by updateWidth() when devicePixelRatio would push the canvas backing
  // store past maxCanvasPixels; hbmedina has no equivalent since it draws
  // lossless SVG paths, not a raster canvas.
...
export class WasmMasahifComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  static DEFAULT_PAGE_SIZE = { width: 255, height: 410, marginWidth: WasmMasahifComponent.DEFAULT_SCALE * 400 };

  pageSize = WasmMasahifComponent.DEFAULT_PAGE_SIZE
  ...
  constructor(@Inject(MUSHAFLAYOUTTYPE) mushafLayoutType: MushafLayoutType,
    ...
    switch (mushafLayoutType) {
      case MushafLayoutType.OldMadinah:
        ...
        this.defaultFontSize = WasmMasahifComponent.DEFAULT_PAGE_SIZE.width / (16400 / 1000);
        break;
      ...
      default:
        ...
        this.defaultFontSize = WasmMasahifComponent.DEFAULT_PAGE_SIZE.width / (16200 / 1000);
        break;
    }
    ...
    this.quranService.promise.then((respone: WasmMushafService) => {
      this.ngZone.runOutsideAngular(async () => {
        ...
        this.pageElements.forEach((page, index) => {
          this.views[index] = new PageView(page.nativeElement, index, this.quranService, this.viewport);
        });
  ...
  private setViewport(scale, updateView: boolean, duringZoom: boolean = false) {
    ...
    if (updateView) {
      this.updateWidth(true, duringZoom);
    }
  }
  ...
  forceRendering(currentlyVisiblePages) {
    ...
    if (pageView) {
      ...
      this.renderView(pageView, this.canvasWidth, this.canvasHeight,
        this.tajweedColorCtrl.value, this.viewport.hasRestrictedScaling);
      return true;
    }
    return false;
  }
  ...
  renderView(view: PageView, canvasWidth, canvasHeight, tajweedColor, hasRestrictedScaling) {
    ...
    switch (view.renderingState) {
      ...
      case RenderingStates.INITIAL:
        this.highestPriorityPage = view;
        view.draw(canvasWidth, canvasHeight, tajweedColor, hasRestrictedScaling)
          .catch(error => {
            console.log(error)
          })
          .finally(() => {
  ...
  navigateToMushaf(layoutIndex) {
    if (layoutIndex === 3) {
      this.router.navigate(['/wasm/indopak15'])
    }
  }
